...
use Botble\Hotel\Tables\Booking\BookingRoomTable;
use Exception;
...
class BookingRoomController extends BaseController
{
    ...
    public function index(BookingRoomTable $table)
    {
        PageTitle::setTitle(trans('plugins/hotel::booking.name') . ' ' . trans('plugins/hotel::booking.room'));
    ...
    public function update(int|string $id, UpdateBookingRequest $request, BaseHttpResponse $response)
    {
        ...
        return $response
            ->setPreviousUrl(route('booking.room.index'))
