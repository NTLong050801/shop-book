...
use Botble\Hotel\Tables\Booking\BookingVoucherTable;
use Exception;
use Illuminate\Http\Request;
use Botble\Base\Facades\PageTitle;
use Botble\Hotel\Http\Controllers\BookingController;

class BookingVoucherController extends BaseController
{
    public function __construct(protected BookingInterface $bookingRepository)
    {
    }

    public function index(BookingVoucherTable $table)
    {
        PageTitle::setTitle(trans('plugins/hotel::booking.name') . ' ' . trans('plugins/hotel::booking.voucher'));
    ...
    public function update(int|string $id, UpdateBookingRequest $request, BaseHttpResponse $response)
    {
        ...
        return $response
            ->setPreviousUrl(route('booking.voucher.index'))
