...
class TourTable extends TableAbstract
{
    ...
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, TourInterface $tourRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $tourRepository;

        if (!Auth::user()->hasAnyPermission(['tour.edit', 'tour.destroy'])) {
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (!Auth::user()->hasPermission('tour.edit')) {
                    return BaseHelper::clean($item->name);
                }

                return Html::link(route('tour.edit', $item->id), BaseHelper::clean($item->name));
            })
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('tour.edit', 'tour.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('tour.create'), 'tour.create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('tour.deletes'), 'tour.destroy', parent::bulkActions());
    }
