@if ($booking)
    <div class="row">
        ...
        <div class="col-md-6">
            <p><strong>{{ __('Voucher') }}</strong>: @if ($booking->voucher->voucher->id) {{ $booking->voucher->voucher->name }} @else N/A @endif</p>
            <p><strong>{{ __('Hotel') }}</strong>: <i>@if ($booking->voucher->voucher->room->id) <a href="{{ $booking->voucher->voucher->room->url }}" target="_blank">{{ $booking->voucher->voucher->room->name }}</a> @else N/A @endif</i></p>
        ...
        <tbody>
            <tr>
                <td class="text-center" style="width: 150px; vertical-align: middle">
                    <a href="{{ $booking->voucher->voucher->room->url }}" target="_blank">
                        <img src="{{ RvMedia::getImageUrl($booking->voucher->voucher->room->image, 'thumb', false, RvMedia::getDefaultImage()) }}" alt="{{ $booking->voucher->voucher->room->name }}" width="140">
                    </a>
                </td>
                <td class="text-center" style="vertical-align: middle">{{ $booking->voucher->voucher->name }}</td>
                <td style="vertical-align: middle"><a href="{{ $booking->voucher->voucher->room->url }}" target="_blank">{{ $booking->voucher->voucher->room->name }}</a></td>
                <td class="text-center" style="vertical-align: middle"><strong>{{ format_price($booking->voucher->voucher->price) }}</strong></td>
