        ...
        $("input.jsDateSinge").daterangepicker(
            {
                ...
                isInvalidDate: function (date) {
                    return !validWeekdays.includes(date.format("dddd").toLowerCase());
                },
            },
        );
    });
...
function updateTotal(formData) {
    ...
    $.ajax({
        ...
        success: function (response) {
            document.getElementById('total').textContent = response.total + ' đ'; // Display total with 2 decimal places
        },
...
$(document).ready(function () {
    ...
    if (rangeSlider) {
        var minPrice = rangeSlider.hasAttribute("data-min-price") ? parseInt(rangeSlider.getAttribute("data-min-price")) : 0;
        var maxPrice = rangeSlider.hasAttribute("data-max-price") ? parseInt(rangeSlider.getAttribute("data-max-price")) : 2000000;
        const minRequest = parseInt(rangeSlider.getAttribute("data-request-min") ? rangeSlider.getAttribute("data-request-min") : minPrice);
        const maxRequest = parseInt(rangeSlider.getAttribute("data-request-max") ? rangeSlider.getAttribute("data-request-max") : maxPrice);
    ...
    $("#slider-range, .amenities, .places, .categories").on("change", function () {
