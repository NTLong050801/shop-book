...
class Theme implements ThemeContract
{
    ...
    public function header(): string
    {
        ...
        //add meta to head
        $metaString = setting('metas_tag');
        // Sử dụng preg_match_all để rút trích tất cả các tag meta từ chuỗi
        preg_match_all('/<meta\s+name="([^"]+)"\s+content="([^"]+)"\s*\/?>/', $metaString, $matches, PREG_SET_ORDER);

        foreach ($matches as $match) {
            $name = $match[1];
            $content = $match[2];
            if ($name !== null && $content !== null) {
                SeoHelper::meta()->addMeta($name, $content);
            }
        }
